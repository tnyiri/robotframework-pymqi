...
Test04
	...
	PyMQI.Put Message    Hello world_t1_1    %{ENV}.SCHRKD.REPLY
	PyMQI.Put Message    Hello world_t1_2    %{ENV}.SCHRKD.REPLY
	PyMQI.Put Message    Hello world_t1_3    %{ENV}.SCHRKD.REPLY
	${msg} =    PyMQI.Get Message    %{ENV}.SCHRKD.REPLY
    Log    Message got back from queue: [${msg}].                                             console=yes
	PyMQI.Purge Queue    %{ENV}.SCHRKD.REPLY

Test05
    [Documentation]     
    Log    Test step: Put 3 message into a queue and read all of them back in one step...     console=yes
	PyMQI.Put Message    Hello world1_t2_1    %{ENV}.SCHRKD.REPLY
	PyMQI.Put Message    Hello world1_t2_2    %{ENV}.SCHRKD.REPLY
	PyMQI.Put Message    Hello world1_t2_3    %{ENV}.SCHRKD.REPLY
	${msgs} =     PyMQI.Get All Messages    %{ENV}.SCHRKD.REPLY
...
Test06
	...
	PyMQI.Get Message Into File    %{ENV}.SCHRKD.REPLY    schrkd_output.msg
...
Test09
	...
	PyMQI.Put Message From File    schrkd_input.msg    %{ENV}.SCHRKD.REPLY
	PyMQI.Put Message From File    schrkd_input.msg    %{ENV}.SCHRKD.REPLY
	PyMQI.Get All Messages Into File    %{ENV}.SCHRKD.REPLY    schrkd_output.msg
    Log    Message got back from queue into file:                                             console=yes
	${File} =    Get File    schrkd_output.msg
	@{list} =    Split to lines  ${File}     
	:FOR    ${line}    IN    @{list}
